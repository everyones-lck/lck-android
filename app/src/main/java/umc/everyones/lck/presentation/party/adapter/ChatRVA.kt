...
/*class ChatRVA :
    PagingDataAdapter<ViewingPartyChatLogModel.ChatLogModel, RecyclerView.ViewHolder>(DiffCallback()) {
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        when(holder) {
            is SenderChatViewHolder -> {
                val sendChat = getItem(position)
                if(sendChat != null) {
                    holder.bind(sendChat)
                }
            }
            is ReceiverChatViewHolder -> {
                val receiveChat = getItem(position)
                if(receiveChat != null) {
                    holder.bind(receiveChat)
                }
            }
        }
    }
    ...
    inner class SenderChatViewHolder(private val binding: ItemSenderChatBinding) :
        ...
        fun bind(chatItem: ViewingPartyChatLogModel.ChatLogModel) {
            with(binding){
                tvSendererChat.text = chatItem.message + bindingAdapterPosition
    ...
    inner class ReceiverChatViewHolder(private val binding: ItemReceiverChatBinding) :
        ...
        fun bind(chatItem: ViewingPartyChatLogModel.ChatLogModel) {
            with(binding){
                tvReceiverChat.text = chatItem.message + bindingAdapterPosition
                /*Glide.with(ivParticipantProfileImage.context)
                    .load(participantItem.profileImage)
                    .into(ivParticipantProfileImage)
    ...
    override fun getItemViewType(position: Int): Int {
        return getItem(position)?.viewType ?: SENDER
    }
